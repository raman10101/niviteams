...
class DbHandler {
    ...
    public function createUser($name, $email, $mobile, $otp) {
        ...
        if (!$this->isUserExists($mobile)) {
			...
			$customer_number = md5($mobile);  
 
            // insert query
            $stmt = $this->conn->prepare("INSERT INTO users(customer_id,name, email, mobile, apikey, status) values(?,?, ?, ?, ?, 0)");
            $stmt->bind_param("sssss", $customer_number, $name, $email, $mobile, $api_key);
    ...
    public function activateUser($otp) {
        $stmt = $this->conn->prepare("SELECT u.id, u.customer_id, u.name, u.email, u.mobile, u.apikey, u.status, u.created_at FROM users u, sms_codes WHERE sms_codes.code = ? AND sms_codes.user_id = u.id");
        $stmt->bind_param("s", $otp);
 
        if ($stmt->execute()) {
            // $user = $stmt->get_result()->fetch_assoc();
            $stmt->bind_result($id, $customer_id, $name, $email, $mobile, $apikey, $status, $created_at);
